...
interface LogViewerTerminalProps extends Omit<
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border/40 bg-muted/30 px-3 py-2">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <ToolbarButton
            ...
            label={copied ? "Copied" : "Copy all logs"}
          >
            {copied ? (
      ...
      <div
        ...
        className="scrollbar-thin overflow-auto bg-card font-mono text-xs leading-relaxed"
        style={{ maxHeight }}
        role="log"
        aria-live="polite"
        aria-label={title}
          ...
          filteredEntries.map((entry, i) => {
            ...
            return (
              ...
              >
                {lineNumbers && (
                  <span
                    className="shrink-0 text-right text-muted-foreground/50 select-none"
                ...
                <span className="min-w-0 flex-1 break-all whitespace-pre-wrap text-foreground/90">
...
interface LogViewerMinimalProps extends Omit<
  React.ComponentProps<"div">,
  "children"
> {
  /** Log entries to display. */
  ...
  return (
    ...
    >
      <div
        ref={scrollRef}
        onScroll={handleScroll}
        className="scrollbar-thin overflow-auto font-mono text-xs leading-relaxed"
        style={{ maxHeight }}
        role="log"
        aria-live="polite"
          ...
          entries.map((entry, i) => {
            ...
            return (
              ...
              >
                <Circle
                  className={cn(
                    "mt-0.75 size-2 shrink-0 fill-current",
                    colors.text
                  )}
...
interface LogViewerFilterableProps extends Omit<
  React.ComponentProps<"div">,
  "children" | "title"
> {
  /** Log entries to display. */
    ...
    >
      ...
      <div className="flex items-center gap-2 border-b border-border/40 bg-muted/30 px-3 py-2">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <ToolbarButton
            onClick={() => exportLogs(filteredEntries)}
            label="Download logs"
          >
            <Download className="size-3.5" />
          </ToolbarButton>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-border/40 bg-muted/10 px-3 py-2">
        ...
        <div className="ml-auto flex items-center gap-1.5 rounded-md border border-border/40 bg-background px-2 py-1">
          ...
          <input
            ...
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Filter…"
            className="w-24 bg-transparent text-xs text-foreground outline-none placeholder:text-muted-foreground sm:w-32"
          />
          {searchQuery && (
            <button
              type="button"
          ...
          filteredEntries.map((entry, i) => {
            ...
            return (
              ...
              >
                <Circle
                  className={cn(
                    "mt-1.25 size-2 shrink-0 fill-current",
                    colors.text
                  )}
                  aria-hidden="true"
                />
